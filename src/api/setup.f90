...
! ──────────────────────────────────────────────────────────────────────────────
module gfnff_api_setup
  use iso_fortran_env,only:wp => real64,stdout => output_unit
  use gfnff_topo_ini,only:gfnff_ini
...
!========================================================================================!
  ...
  subroutine gfnff_setup(nat,at,xyz,ichrg,printlevel,restart,write_topo,preset_param, &
  &                      gen,param,topo,neigh,cell,accuracy,version,io,printunit)
    ...
    !*   preset_param - gen/param already populated by the caller; skip
    !*                  the internal parameter load
    !*   printunit   - output unit (optional, default: stdout)
    !* Output:
    !*   io          - error status (0 = success)
    !***********************************************************************
    use gfnff_restart,only:read_restart_gff,write_restart_gff
...
!> Dummy
    ...
    !> .true. when gen and param were supplied by the caller (e.g. read from a
    !> TOML parameter set) and must not be overwritten from the internal tables
    logical,intent(in) :: preset_param
    integer,intent(out) :: io
    integer,intent(in),optional :: printunit  !< output unit (default: stdout)

!> Stack
    integer :: newichrg,myunit
    !> no external field from this entry point; gfnff_ini takes one
    !> because the energy evaluation does, but setup never applies it
    real(wp),parameter :: efield(3) = 0.0_wp
...
!> Run initialization
! ──────────────────────────────────────────────────────────────────────────────
    !> .true.: the neighbour list is always built here. This used to travel
    !> from gfnff_input through a module global that nothing else read.
    call gfnff_ini(printlevel,.true.,nat,at,xyz,ichrg, &
    &                  gen,param,topo,neigh,cell,efield, &
    &                  accuracy,version,io,printunit=myunit)
