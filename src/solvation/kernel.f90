...
contains
  ...
  subroutine addGradientSaltStill(nat,ntpair,ppind,ddpair,qat,kappa, &
    ...
    real(wp) :: aa,r2,fgb
    real(wp) :: qq,dd,expd,dfgb,dfgb3,egb,ap,bp,qfg
    real(wp) :: gg,expa
  ...
  subroutine addGradientP16(nat,ntpair,ppind,ddpair,qat,keps, &
        & brad,brdr,energy,gradient)
    !***********************************************************************
    !* P16 Born energy and its gradient, both the direct distance part and
    !* the part that goes through the Born radii.
    !*
    !* Unlike the matrix build, the pair loop here accumulates into shared
    !* atom-indexed arrays, so it needs a reduction. That makes the result
    !* depend on the thread count in the last bits - the same tolerance the
    !* rest of the GFN-FF gradient already carries. The reduced arrays are
    !* of length nat and 3*nat, so the per-thread copies are cheap.
    !***********************************************************************
    ...
    !$omp reduction(+:egb,gradient,dEdbr) &
    !$omp private(kk,iat,jat,vec,r1,r2,ab,arg1,arg16,fgb,dfgb,dfgb2,ap, &
    !$omp&        bp,qq,dEdbri,dEdbrj,dG) &
  ...
  pure subroutine addBornDerivStill(nat,ntpair,ppind,ddpair,qat,keps, &
    ...
    real(wp),parameter :: sqrt2pi = sqrt(2.0_wp/pi)
    real(wp) :: aa,r2,fgb2
    real(wp) :: qq,dd,expd,dfgb,dfgb2,dfgb3,ap,bp
    real(wp) :: egb
    real(wp) :: grddbi,grddbj
    real(wp) :: dr(3),r
  ...
  subroutine addBornMatP16(nat,ntpair,ppind,ddpair,keps,brad,Amat)
    !***********************************************************************
    !* P16 Born interaction matrix. ppind holds every unordered pair once,
    !* so an iteration owns both Amat(i,j) and Amat(j,i) outright and the
    !* pair loop parallelises with no reduction and no reordering: each
    !* element still receives exactly one addition, whatever the schedule.
    !***********************************************************************
    ...
    integer :: kk
    integer :: iat,jat
    real(wp) :: r1,ab,arg,fgb,dfgb,bp

    !$omp parallel do default(none) schedule(static) &
    !$omp shared(Amat,ntpair,ppind,ddpair,brad,keps) &
