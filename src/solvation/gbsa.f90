...
module gfnff_solv_gbsa
  use iso_fortran_env,only:wp => real64
  use gfnff_math_wrapper,only:matDet3x3,dot,gemv,symv

  use gfnff_search,only:bisectSearch
  use gfnff_solv_born,only:compute_bornr,split_range
  use gfnff_solv_kernel,only:gbKernel,addBornMatSaltStill,addBornMatStill, &
     & addBornMatP16,addGradientSaltStill,addGradientStill,addGradientP16, &
     & addBornDerivSaltStill,addBornDerivStill
  use gfnff_solv_lebedev,only:gridSize,getAngGrid
  use gfnff_solv_sasa,only:compute_numsa
  use gfnff_solv_state,only:solutionState

  use gfnff_solv_type,only:TSolvation
  implicit none
  private
  ...
  type,extends(TSolvation) :: TBorn
    ...
    !> the descreening integral the Born radii are built from, kept because
    !> the closed-form Hessian needs the rescaling derivatives at this psi and
    !> compute_bornr otherwise overwrites it in place
...
!  OMP parallel version.
  subroutine update_nnlist_gbsa_parallel(nat,ntpair,ppind,xyz,lrcut, &
    ...
    !***********************************************************************
    !* Pair distances plus the two neighbour lists, in parallel.
    !*
    !* Done as a counting sort over contiguous slices of the pair list: one
    !* pass computes every pair distance and counts, per slice, how many
    !* entries that slice will contribute to each list, a prefix sum turns
    !* those counts into write offsets, and a second pass fills the lists.
    !* Since the slices are contiguous and taken in order, every list comes
    !* out in exactly the order the sequential routine produces - the two
    !* are interchangeable to the bit.
    !*
    !* The earlier version instead gave every thread its own nat x nat
    !* scratch list, which costs more memory than the lists being built.
    !* Here the only per-thread storage is one count per atom.
    !***********************************************************************
!$  use omp_lib,only:omp_get_max_threads
    ...
    integer :: kk,i1,i2,t,nproc,lo,hi,run
    real(wp) :: lrcut2,srcut2
    real(wp) :: x,y,z,dr2
    integer,allocatable :: nrad(:),radoff(:)
    integer,allocatable :: cnts(:,:),sasoff(:,:)
    integer :: pos(nat),ip
...
!$  nproc = omp_get_max_threads()
    nproc = max(1,min(nproc,max(1,ntpair)))
    allocate (nrad(nproc),radoff(nproc))
    allocate (cnts(nat,nproc),sasoff(nat,nproc))

    ! ── distances for every pair, and per-slice counts for both lists ───────
    !$omp parallel do default(none) schedule(static) &
    !$omp shared(nproc,ntpair,ppind,xyz,ddpair,lrcut2,srcut2,nrad,cnts,nat) &
    !$omp private(t,lo,hi,kk,i1,i2,x,y,z,dr2,ip)
    do t = 1,nproc
      call split_range(ntpair,nproc,t,lo,hi)
      cnts(:,t) = 0
      ip = 0
      do kk = lo,hi
        i1 = ppind(1,kk)
        ...
        if (dr2 .lt. lrcut2) then
          ...
          if (dr2 .lt. srcut2) then
            cnts(i1,t) = cnts(i1,t)+1
            cnts(i2,t) = cnts(i2,t)+1
          end if
        end if
      end do
      nrad(t) = ip
    ...
    ! ── prefix sums: where each slice starts writing in each list ───────────
    run = 1
    do t = 1,nproc
      radoff(t) = run
      run = run+nrad(t)
    ...
    ! ── fill both lists; no two slices ever write the same slot ─────────────
    !$omp parallel do default(none) schedule(static) &
    !$omp shared(nproc,ntpair,ppind,xyz,lrcut2,srcut2,radoff,sasoff, &
    !$omp&       nnlistr,nnlists,nat) &
    !$omp private(t,lo,hi,kk,i1,i2,x,y,z,dr2,ip,pos)
    do t = 1,nproc
      call split_range(ntpair,nproc,t,lo,hi)
      ip = radoff(t)
      pos = sasoff(:,t)
      do kk = lo,hi
        ! recomputed, not read back from ddpair: a round trip through sqrt
        ! could put a pair on the other side of the cut-off and desynchronise
        ! the two passes
        i1 = ppind(1,kk)
        i2 = ppind(2,kk)
        x = xyz(1,i1)-xyz(1,i2)
        ...
        if (dr2 .lt. lrcut2) then
          ...
          if (dr2 .lt. srcut2) then
            pos(i1) = pos(i1)+1
            nnlists(pos(i1),i1) = i2
            pos(i2) = pos(i2)+1
            nnlists(pos(i2),i2) = i1
          end if
        end if
      end do
    end do
...
!  Sequential version.
  pure subroutine update_nnlist_gbsa_sequential(nat,ntpair,ppind,xyz,lrcut, &
    ...
    ! nnlists and nnlistr are only ever read up to nnsas(i) and nnrad, so the
    ! tail is never seen; blanking them costs 23 MB of memset at 1500 atoms
    ! and buys nothing
    nnsas = 0
    ip = 0
    ip2 = 0
