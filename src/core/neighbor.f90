...
module gfnff_neighbor
  use iso_fortran_env,only:wp => real64,stdout => output_unit,int8
  use gfnff_data_types,only:TGFFData,TCell
  use gfnff_geometry,only:lin
  use gfnff_search,only:indexHeapSort
  implicit none
  private
  ...
  !> Neighbor lists
  !> Single source for the two neighbour-list bounds. They are parameters
  !> rather than literals because jth_nb sizes a fixed-size local from their
  !> product, and a fixed size is the point: a runtime-sized automatic array
  !> is heap-allocated by gfortran, which is what that local exists to avoid.
  !> Raising either constant therefore has to grow that scratch too, and
  !> single-sourcing them is what makes that automatic.
  integer,parameter :: max_numnb = 42
  integer,parameter :: max_numctr = 27   !> the 3x3x3 block of central cells
  integer,parameter :: jth_nb_scratch = max_numnb*max_numctr
  ...
  type :: TNeigh
    ...
    !> one byte is enough: this counts covalent bonds along the shortest path
    !> and saturates at 5, while at 1000 atoms a default integer would spend
    !> 114 MB on it
    integer(int8),allocatable :: bpair(:,:,:)
    ...
    ! optional host-supplied molecular graph (nat,nat) of integer bond orders.
    ! When allocated it replaces the distance criterion in get_nb, so the
    ! connectivity no longer follows from the coordinates. See TGFFUserInput.
    integer,allocatable :: user_bondmat(:,:)
  contains
    ! initialize neigh
    procedure :: init_n
...
! ══════════════════════════════════════════════════════════════════════════════
  ...
  subroutine get_nb(self,nat,at,xyz,cell,rab,rtmp,mchar,icase,f_in,f2_in,param)
    ...
    !> A host-supplied graph replaces the distance criterion outright, and all
    !> three neighbour lists become that graph. The filters fillnb applies for
    !> icase 2 and 3 (hypercoordination, metals) exist to undo the distance
    !> perception's own over-perception; there is nothing here to correct.
    if (allocated(self%user_bondmat)) then
      call fillnb_from_graph(self,nat,icase)
      return
    end if
 ...
 write (stdout,'("**ERROR**",a,1x,a)') 'Implementation is not adjusted for such small cells.',source
  ...
  subroutine fillnb_from_graph(self,n,icase)
    !***********************************************************************
    !* Fill one neighbour list from the host-supplied molecular graph instead
    !* of from interatomic distances. Same packing as fillnb: entries 1..nn
    !* hold the neighbour indices and entry numnb holds their count.
    !*
    !* Only the zero/nonzero pattern of the matrix is read; the bond orders
    !* it carries have no consumer yet. Bonds go into the central cell only,
    !* which is why the graph is restricted to molecular systems.
    !* Input:
    !*   n     - number of atoms
    !*   icase - which list to fill, 1 = nbf, 2 = nb, 3 = nbm
    !***********************************************************************
    implicit none
    ...
    select case (icase)
    case (1)
      if (.not.allocated(self%nbf)) allocate (self%nbf(self%numnb,n,self%numctr))
      self%nbf = 0
    case (2)
      if (.not.allocated(self%nb)) allocate (self%nb(self%numnb,n,self%numctr))
      self%nb = 0
    case (3)
      if (.not.allocated(self%nbm)) allocate (self%nbm(self%numnb,n,self%numctr))
      self%nbm = 0
    end select
    ...
    do i = 1,n
      ...
      do j = 1,n
        if (j .eq. i) cycle
        if (self%user_bondmat(j,i) .eq. 0) cycle
        nn = nn+1
        select case (icase)
        case (1); self%nbf(nn,i,1) = j
        case (2); self%nb(nn,i,1) = j
        case (3); self%nbm(nn,i,1) = j
        end select
      end do
      select case (icase)
      case (1); self%nbf(self%numnb,i,1) = nn
      case (2); self%nb(self%numnb,i,1) = nn
      case (3); self%nbm(self%numnb,i,1) = nn
      end select
    end do

  end subroutine fillnb_from_graph

  ! is a modified "getnb" from topo/neighborlist.f90
...
!           no metals and unusually coordinated stuff
  ...
  subroutine jth_nb(self,n,xyz,j,jth,i,iTr)
    ...
    !> Automatic, not allocatable. This routine is called once per hydrogen
    !> bond candidate per gradient evaluation -- hundreds of thousands of
    !> times for a few hundred atoms -- and the two heap allocations cost
    !> more than the work they hold. numnb*numctr bounds the neighbour count
    !> across all cells, so the stack copy is never smaller than sizenb.
    integer :: indx(jth_nb_scratch)
    real(wp) :: distnb(jth_nb_scratch)

    sizenb = sum(self%nb(self%numnb,i,:))
    !> the scratch is fixed size; refuse rather than overrun it
    if (sizenb > jth_nb_scratch) error stop 'jth_nb: neighbour count exceeds jth_nb_scratch'
    distnb(1:sizenb) = 0.0_wp
    indx(1:sizenb) = 0
    ...
    call indexHeapSort(indx(1:sizenb),distnb(1:sizenb))
    l = 0
    do iTrdum = 1,self%numctr
      do k = 1,self%nb(self%numnb,i,iTrdum)
        jdum = self%nb(k,i,iTrdum)
