...
!> which must be included via a suitable library.
  ...
  !> Generic wrappers, single and double precision resolved by the argument
  !> kind. The LAPACK/BLAS interface blocks themselves stay private: a caller
  !> should reach the library through the wrapper, which is what checks the
  !> info code and the array shapes.
  public :: contract
  public :: dot,symv,gemv,gemm
  public :: sytrf_wrap,sytrs_wrap
  public :: matDet3x3
