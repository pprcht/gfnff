...
!> Fixed GFN-FF parameter tables: the force field version enumerator, the
!> periodic system, and the per-element constants the parametrisation is built
!> from. Nothing here is computed -- gfnff_param turns these into a TGFFData
!> for a particular system.
!>
!> The arrays are indexed by atomic number and run to element 103, or to 118
!> where the source data goes that far, so element Z reads entry Z with no
!> mapping in between.
module gfnff_param_tables
  use iso_fortran_env,only:wp => real64
  implicit none
  !> Blanket public, deliberately: this module is nothing but GFN-FF parameter
  !> constants. There is no implementation to encapsulate, and an
  !> explicit export list would be a second copy of the declarations
  ...
  type :: TGFFVersionEnum
    ...
    !> angewChem2020_2 with the dissociative tail of the bond term replaced
    !> by a rising continuation, for conformer work where bonds must not break
    integer :: conformer2020 = 5
...
!----------------------------------------------------------------------------------------
 ...
 &                  6.7286_wp, 6.5144_wp,10.9169_wp,10.3600_wp, 9.4723_wp, 8.6641_wp ] ! Nh-Og
  ...
  real(wp),parameter :: sqrtZr4r2(1:118) = &
