...
!========================================================================================!
  ...
  subroutine newD3Model(dispm,nat,at)
    !> wholesale: pure reference data, read by the model builder below
    use gfnff_dftd3param
    use gfnff_dftd4param
    implicit none
...
!========================================================================================!
  ...
  subroutine d3_gradient(dispm,nat,at,xyz,npair,pairlist,zeta_scale,radii, &
    ...
    integer :: max_ref
    integer :: iat,jat,ati,atj,ij,img,iref,jref
    real(wp) :: r4r2ij,r0,rij(3),r2,t6,t8,t10,d6,d8,d10
    real(wp) :: dE,dG(3),disp,ddisp

    real(wp) :: c6ij,dc6i,dc6j,refc6
    real(wp),allocatable :: gw(:,:),dgwdcn(:,:)
    real(wp),allocatable :: energies(:),dEdcn(:)

    max_ref = maxval(dispm%nref(at))
    !> no (nat,nat) C6 matrix: the coefficients are built per pair inside the
    !> loop below, which also skips the pairs outside the dispersion cutoff
    allocate (gw(max_ref,nat),dgwdcn(max_ref,nat), &
    ...
    !$omp reduction(+:energies, gradient, dEdcn) &
    !$omp shared(at, xyz, npair, pairlist, zeta_scale, dispscale, radii, r4r2, &
    !$omp&       dispm, gw, dgwdcn) &
    !$omp private(ij, img, iat, jat, ati, atj, r2, rij, r4r2ij, r0, t6, t8, t10, &
    !$omp&        d6, d8, d10, disp, ddisp, dE, dG, iref, jref, refc6, c6ij, dc6i, dc6j)
...
!DIR$ NOVECTOR
    do img = 1,npair
      ...
      !> C6 and its CN derivatives for this pair only
      c6ij = 0.0_wp
      dc6i = 0.0_wp
      dc6j = 0.0_wp
      do iref = 1,dispm%nref(ati)
        do jref = 1,dispm%nref(atj)
          refc6 = dispm%c6(iref,jref,ati,atj)
          c6ij = c6ij+gw(iref,iat)*gw(jref,jat)*refc6
          dc6i = dc6i+dgwdcn(iref,iat)*gw(jref,jat)*refc6
          dc6j = dc6j+gw(iref,iat)*dgwdcn(jref,jat)*refc6
        end do
      end do
      ...
      dEdcn(iat) = dEdcn(iat)-dc6i*disp
      if (iat /= jat) then
        energies(jat) = energies(jat)+dE
        dEdcn(jat) = dEdcn(jat)-dc6j*disp
...
!========================================================================================!
  ...
  subroutine disp_gradient_latp(nat,at,xyz,fraglist,ntrans,trans,zeta_scale,radii,cutoff,par,r4r2,c6,dc6dcn, &
    ...
    do iat = 1,nat
      ati = at(iat)
      do jat = 1,iat
        !> calc_inter=.true. selects SAME-fragment pairs, i.e. the opposite of
        !> what the name says and of what the mcGFN-FF paper describes (it
        !> adjusts s8 for atoms in different molecules). Copied verbatim from
        !> xtb, where disp_gradient_latp_inter/_intra carry the same swap, and
        !> the mc factors were fitted against that code. Do not "fix" without
        !> refitting: it would break agreement with the published X23 results.
