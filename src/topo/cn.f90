...
!  function to return the respective CN.
  subroutine ncoordLatP(nat,at,xyz,ntrans,trans,cutoff,kcn,cfunc,dfunc,enscale, &
    ...
    ! ── own-row form: the thread that owns iat writes only iat's slots ─────────
    ! The triangular loop this replaces wrote into both partners of every pair
    ! and therefore needed reduction(+:dcndr) on a (3,nat,nat) array, which
    ! costs a private copy per thread. Here every thread owns one atom, so no
    ! reduction is needed at the price of visiting each pair from both ends.
    ! The lattice point set is closed under negation, so summing over all jat
    ! and all translations reproduces the triangular sum exactly, except for
    ! the self pair jat == iat, which the old loop counted through both
    ! cn(iat) and cn(jat) and which is therefore weighted twice below.
    !$omp parallel do default(none) private(den) shared(enscale, rcov, en) &
    !$omp shared(nat,at,xyz, kcn, trans, cutoff2, ntrans, cn, dcndr, dcndL) &
    !$omp private(iat, jat, itr, ati, atj, r2, rij, r1, rc, countf, countd, stress) &
    ...
    do iat = 1,nat
      ...
      do jat = 1,nat
        ...
        do itr = 1,ntrans
          ...
          if (jat .eq. iat) then
            !> self image: no net gradient, and counted twice as above
            cn(iat) = cn(iat)+2.0_wp*countf
            dcndL(:,:,iat) = dcndL(:,:,iat)+2.0_wp*stress
          else
