...
module gfnff_rab
  ...
  !> exported for the equivalence test in test/tools; the table it replaced
  !> was module data that a test could read directly
  public :: scaleF
...
! ══════════════════════════════════════════════════════════════════════════════
  ...
  !> NOTE: the literals are deliberately left at default (single) precision.
  !> The original code assigned them element-wise from unsuffixed literals,
  !> so the stored values carry float32 rounding; adding _wp here would
  !> silently change every bond reference length in the last digits.
  real(wp),parameter :: pEN(6,2) = reshape([ &
    &  29.84522887,-1.70549806,6.54013762,6.39169003, &
    &   6.00000000,5.60000000, &   ! handish downward extrapolated for GFN-FF
    &  -8.87843763,2.10878369,0.08009374,-0.85808076, &
    &  -1.15000000,-1.30000000], [6,2])

  !> Halogens whose bonds to a lanthanide or actinide are scaled, and by how
  !> much. Everything not listed here scales by one; see scaleF below.
  !>
  !> The literals carry no kind suffix on purpose. They are default real, and
  !> what the model was fitted against is their promotion to double -- adding
  !> _wp would change the last bits of every Ln/Ac halogen bond length.
  integer,parameter :: lnac_halogen(4) = [9,17,35,53]
  real(wp),parameter :: lnac_scale(4) = [0.93533568,1.0190114,1.0425532,1.0551948]

! ══════════════════════════════════════════════════════════════════════════════
contains  !> MODULE PROCEDURES START HERE
! ══════════════════════════════════════════════════════════════════════════════

  pure function scaleF(ati,atj) result(sf)
    !***********************************************************************
    !* Bond length scaling for a halogen bonded to a lanthanide or actinide.
    !* One for every other pair.
    !*
    !* This used to be a 103x103 table filled lazily on first call behind a
    !* check-then-set flag, which made it shared mutable state between
    !* otherwise independent calculators and a data race between threads. The
    !* table was one everywhere except eight blocks, so the lookup is cheaper
    ...
    sf = 1.0_wp
    do k = 1,size(lnac_halogen)
      if (is_lnac(ati).and.atj .eq. lnac_halogen(k)) sf = lnac_scale(k)
      if (is_lnac(atj).and.ati .eq. lnac_halogen(k)) sf = lnac_scale(k)
    end do
  end function scaleF

  !> lanthanide (57-71) or actinide (89-103)
  pure function is_lnac(at) result(yes)
    implicit none
    integer,intent(in) :: at
    logical :: yes
    yes = (at >= 57.and.at <= 71).or.(at >= 89.and.at <= 103)
  end function is_lnac

  subroutine gfnffdrab(n,at,cn,nsrb,srblist,rab,gfac,rabdcn)
    !***********************************************************************
    !* CN-dependent reference bond lengths and the factors needed for their
    !* derivatives.
    !* Input:
    !*   n/at      - system definition
    !*   cn        - coordination numbers
    !*   nsrb      - number of bonds
    !*   srblist   - bond list, (1,:) and (2,:) are the two atoms
    !* In/out:
    !*   rab       - in: predetermined bond shifts, out: reference lengths
    !* Output:
    !*   gfac      - per bond (scaleF*ff, cnfak(ati), cnfak(atj)); the caller
    !*               rebuilds drab/dr_m = gfac(1)*(gfac(2)*dcn(:,m,i)
    !*               + gfac(3)*dcn(:,m,j)) on the fly, which avoids
    !*               materialising an (3,n,nsrb) array
    !*   rabdcn    - derivative of rab w.r.t. the two coordination numbers
    !***********************************************************************
    implicit none
    ...
    real(wp) :: gfac(3,nsrb)       ! output bond length gradient factors
    real(wp) :: rabdcn(2,nsrb)     ! output bond lengths derivative w.r.t. cn

!     local variables
    integer i,j,k,ati,atj,ir,jr

    real(wp) :: ra,rb,k1,k2,den,ff,sf
    ...
    do k = 1,nsrb
      ...
      k1 = 0.005d0*(pEN(ir,1)+pEN(jr,1))
      k2 = 0.005d0*(pEN(ir,2)+pEN(jr,2))
      ff = 1.0d0-k1*den-k2*den**2
      sf = scaleF(ati,atj)
      rab(k) = (ra+rb+rab(k))*ff*sf
      rabdcn(1,k) = cnfak(ati)*ff*sf
      rabdcn(2,k) = cnfak(atj)*ff*sf
      ! factors for drab/dr_m, kept in the same grouping as before so that
      ! the reconstructed gradient is bit-identical to the materialised one
      gfac(1,k) = sf*ff
      gfac(2,k) = cnfak(ati)
      gfac(3,k) = cnfak(atj)
      !--------
